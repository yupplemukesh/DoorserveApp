...
namespace TogoFogo.Models
{
    ...
    public class TreeView<T> : IHtmlString
    {
        ...
        private Func<T, int> _ValueProperty = item => Convert.ToInt32(item);
        ...
        public TreeView<T> ItemValue(Func<T, int> selector)
        {
            if (selector == null) throw new ArgumentNullException("selector");
            _ValueProperty = selector;
            return this;
        }
        ...
        private TagBuilder GetLi(T item)
        {
            ...
            {
                if (prop.Name.ToLower() == "MenuCapId")
                    li.MergeAttribute("MenuCapId", prop.GetValue(item, null).ToString());
