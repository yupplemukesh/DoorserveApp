...
namespace doorserve.Controllers
{
    public class DropdownBindController : Controller
    {
        ...
        public List<ListItem> BindLocationforEmp(int? LocationId)
        {
            using (var con = new SqlConnection(_connectionString))
            {
                List<ContactPersonModel> company = con
                    .Query<ContactPersonModel>(
                        "select distinct LocationId, LocationName  from MstLocation where LocationId=@LocationId", new { @LocationId = LocationId },
                        commandType: CommandType.Text).ToList();
                List<ListItem> items = new List<ListItem>();
                foreach (var val in company)
                {
        ...
        public JsonResult BindCityByPincode(string value)
        {
            ...
            return Json(distt, JsonRequestBehavior.AllowGet)
 ;
        ...
        public JsonResult BindSatetByPincode(string value)
        {
            using (var con = new SqlConnection(_connectionString))
            {
                ...
                foreach (var val in rmid)
                {
                    items.Add(new ListItem
                    {
                        Value = val.StateId.ToString(), //Value Field(ID)
                        Text = val.State  //Text Field(Name)
                    });
                }
                return Json(items, JsonRequestBehavior.AllowGet);
            }
